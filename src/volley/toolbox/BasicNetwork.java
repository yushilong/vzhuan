    ...
    @SuppressWarnings("unused") @Override public NetworkResponse performRequest(Request<?> request) throws VolleyError
    {
        ...
        while (true)
        {
            ...
            try
            {
                ...
                Cache.Entry rcEntry = B5MVolley.getRequestCache().get(request.getCacheKey());
                addCacheHeaders(headers, request.getCacheEntry());
                if (request.shouldCache() && rcEntry != null && !rcEntry.isExpired() && !request.getRefreshCacheNeeded())
                {
                    return new NetworkResponse(HttpStatus.SC_OK, rcEntry.data, responseHeaders, true);
                }
            ...
            {
                VolleyLog.e("soket timeout ===>retry");
                attemptRetryOnException("socket", request, new TimeoutError());
            }
            catch (ConnectTimeoutException e)
            {
                VolleyLog.e("connect timeout ===>retry");
